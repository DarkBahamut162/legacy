...
--for pn in ivalues(PlayerNumber) do
	local MetricsName = "SongMeterDisplay";
	local songMeterDisplay = Def.ActorFrame{
		...
		Def.SongMeterDisplay {
			...
			Tip=LoadActor( THEME:GetPathG( 'SongMeterDisplay', 'tip ' .. PlayerNumberToString(PLAYER_1) ) ) .. { InitCommand=function(self) self:visible(false) end; };
		};
	};
	--if ThemePrefs.Get("TimingDisplay") == true then
		songMeterDisplay[#songMeterDisplay+1] = CreateSegments();
	--end
